...
- 2026-09-20 (WoPeD fork): IN PROGRESS — user asked whether the README explains `make -C`; it used the flag everywhere but never explained it. Added an explanation under "Make targets" (change into `images/` before reading the Makefile, equivalent to `cd images && make`) and a pointer from the Quick start.
- 2026-09-20 (WoPeD fork): DONE — the user confirmed in chat that the browser check passed (the remaining item: UI in the browser and setting `/nets` as WoPeD's home directory). All Verification items now have a recorded result: image builds on JDK 17 and 21, WoPeD starts and its window reaches Xpra, password enforced on the TCP/WebSocket port, `/nets` host folder, settings persistence, exit on close, Makefile targets, and the without-Docker targets. Delivered in commits dd244d793, 7b478af04 and a8ead53cb on `hoodox/WoPeD`. Not done / left open on purpose: TLS reverse proxy and hosting (Scope: out), multi-user or shared sessions (default is one container per session), JDK 25 (untried), the missing `libx264` encoder (Xpra falls back, only add it if lag is noticed), and the local `woped-xpra:21` image predates the `/nets` warning (rebuild with `make -C images build WOPED_JAVA_VERSION=21 WOPED_IMAGE=woped-xpra:21`).
